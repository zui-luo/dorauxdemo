...
- Dora 首页以“华东销售经营日报”为业务旅程，展示可点击 KPI、趋势切换、异常下钻、Dora 今日洞察和预填问题 Sender。
...
- 所有 Sender 均支持语音输入，点击停止会将识别结果回填到当前 Sender 并关闭语音 Sheet，也可直接发送。
...
| 默认组合 | 默认展示“方案B：更多菜单”与“方案3：名称+标题” |
...
| 经营 KPI | 展示昨日销售额、新增商机、回款完成率、风险客户；点击任一 KPI 后，下方趋势面板切换为该指标的手机端下钻视图 |
| 趋势下钻 | 支持 7 天 / 30 天周期切换，柱状趋势突出异常点，并展示按区域、客户或风险类型聚合的前三项明细 |
| Dora 今日洞察 | 展示销售下滑、商机转化、回款异常；洞察卡只承载判断，不直接触发发送 |
| 首页 Sender | 完全复用会话页 Sender 样式，预填基于洞察的建议问题，支持添加上下文、语音输入和发送；添加菜单将拍照、图片、本地文件归为“手机本地”，将 FineBI、FineReport 归为“数据连接器”；Dora 建议与 Sender 共同固定在底部输入区，通过背景模糊渐变与上方内容柔和过渡 |
...
| 语音停止 | Stopped | 停止计时和转写，将当前识别文本回填到对应输入框，并自动关闭语音 Sheet |

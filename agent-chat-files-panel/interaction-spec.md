...
新消息提醒与“上次停留页”分离处理。Agent 定时任务完成后，如果用户不在对应 Agent 页面内，左侧一级导航、专家卡片和对应最近会话都会出现数字标记；这些数字标记只提示未读状态，不主动改写用户离开专家模块时停留的页面。用户点击提醒所在的 Agent 或会话时，才进入对应会话并消费该提醒。
...
| Agent 卡片出现数字标记 | 表示该专家 Agent 下有定时任务完成后的未读新消息 |
| 点击带数字标记的 Agent 卡片 | 进入该 Agent 对应会话页，并在最近会话中定位到带未读的会话 |
| 点击去创建 | 进入创建 Agent 入口 |
...
| 最近会话出现数字标记 | 表示该会话有定时任务完成后推送的新消息 |
| 点击带数字标记的最近会话 | 打开该会话并清除对应未读状态 |
